...
def location_losses(pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    #print("Shape of target after normalization:", target.shape)
    #print("Shape of pred_locs:", pred.shape)
    ...
    def train_pred_prober(self):
        ...
        for epoch in tqdm(range(epochs), desc=f"Probe prediction epochs"):
            for batch in tqdm(dataset, desc="Probe prediction step"):
                ...
                # init_states = batch.states[:, 0:1]  # BS, 1, C, H,
                # pred_encs = model(states=init_states, actions=batch.actions, training=False)
                
                # # Transpose predictions to required shape [T, BS, D]
                # pred_encs = pred_encs.transpose(0, 1)  # Shape: (T, BS, D)
                ...
                #print("Shape of init_states:", init_states.shape) 
                ...
                #print("Shape of pred_encs (T, BS, D):", pred_encs.shape)
                ################################################################################
